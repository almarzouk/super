...
if (isset($_POST['confirm_emprunt'])){$confirm_emprunt=$_POST['confirm_emprunt'];} else{$confirm_emprunt='off';}
...
$stmt=$pdo->prepare('INSERT INTO `emprunts`( `id_emprunteur`, `id_objet`, `date_emprunt`, `date_restitution`,`encours`) VALUES (:id_emprunteur,:id_objet,:date_emprunt,:date_restit,:encours)');
			...
			if($stmt->execute())
				{	
					...
					header("Refresh:$refresh;../landing.php");
				// header("Location: new.php?info=ok&id_stag=$id_stag&label_change=green");
				}
				else{
					echo "<p>Insertion non faite dans la BDD.</p>";
					header("Refresh:$refresh;new.php?info=ko&label_change=red");
				//	header("Location: new.php?info=ko&id_stag=$id_stag&label_change=red");
					}
